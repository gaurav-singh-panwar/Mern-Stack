               ...
               const userLogin =  router.post('/login',async(req, res)=>{
                    ...
                    try {
                            ...
                            if(userLogin){
                                ...
                                if(!isMatch){
                                    res.status(400).json({error:"Invalid Credentials 1234"});
                                }
                                else if(isMatch){
                ...
                const postBlog = router.post('/postBlog',async(req, res)=>{
                    ...
                    if(!heading || !blog || !userID ){
                        return res.status(422).json({error:"Please Fill details of postBlog"});
                    }
                    try {
                    // else{
                        const user = new Blog({heading,blog,userID});
                        const userReister = await user.save();
                        res.status(201).json({message:"Successfully Registered Blog"});
                        // return res.json({Heading:req.body.heading , Blog:req.body.blog, UserID:req.body.userID});
                    }catch (error) {
                        console.log(error);
                    ...
                    try{
                        ...
                        if(blogExist){
                          console.log(blogExist.heading);
                            return res.send({BlogHeading: blogExist.heading, BlogContent: blogExist.blog} );
                        }
